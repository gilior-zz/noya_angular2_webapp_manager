...
@Component({ selector: 'press', template: require('./press.html!text') })
...
export class PressComponent implements OnInit, OnDestroy {
    ...
    addNew() {
        this.router.navigate(['PressEditor']);
    }
    updatePressItems() {

        this.pressItems.forEach((item) => {
            var id = 'editor_eng_press_' + item.ID;
            var editorEng = CKEDITOR.instances[id];
            id = 'editor_heb_press_' + item.ID;
        ...
        var req: model.UpdatePressRequest = { PressItems: this.pressItems };
        this.dataService.ConnectToApiData(req, 'api/Data/UpdatePress').subscribe(
    ...
    toCKEDITOR(index: number) {
        ...
        if (index == this.pressItems.length - 1) {
            this.pressItems.forEach((item) => {
                var engID = 'editor_eng_press_' + item.ID;
                var hebID = 'editor_heb_press_' + item.ID;
    ...
    loadItems(updateVariables: boolean) {
        ...
        this.dataService.ConnectToApiData(req, 'api/Data/GetPress').subscribe(
            (res: model.PressResponse) => {
                this.pressItems = res.PressItems;
                if (updateVariables) {
